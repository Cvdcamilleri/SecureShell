...
while 1:
    ...
    sys.stdout = _stdout(conn)
    sys.stdin = _stdin(conn)

    try:
      AUTH = raw_input("AUTH")
      #sys.stderr.write(repr(AUTH)+str("\n"))
      #sys.stderr.write(repr(hashlib.sha512(PASSWORD).hexdigest()) + str("\n"))
    except Exception as e:
      sys.stderr.write(str(e))

    if hashlib.sha512(PASSWORD).hexdigest() not in AUTH:
	LOGIN = False
    else:
#	sys.stdout = _stdout_secure(conn)
	LOGIN = True

    RESPONSE = "Remote Shell MK:1"

    if (LOGIN == True):

	    while 1:
		try:
			ENC = str(raw_input(RESPONSE))
			DEC = decrypt_blob(ENC[:-1]+"=", PRIVKEY)
			if (DEC == "exit"):
				break
			sys.stderr.write(decrypt_blob(ENC[:-1]+"=", PRIVKEY)+"\n")
			try:
	...
	print "AUTHENTICATION ERROR, QUITTING"
